...
from telegram.constants import ParseMode # Ensure this is imported
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Ensure ParseMode.MARKDOWN_V2 is used here
        await update.message.reply_text(gemini_response, parse_mode=ParseMode.MARKDOWN_V2) 

    except Exception as e:
        logger.error(f"Error interacting with Gemini for chat_id {chat_id}: {e}")
        if "Can't parse entities" in str(e):
            await update.message.reply_text(
                "Oops! I encountered an error formatting Gemini's response. "
                "It might contain characters that Telegram's Markdown doesn't like. "
                "Here's the raw response (might look a bit odd):\n\n" + gemini_response
            )
        else:
            await update.message.reply_text(
